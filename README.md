...
כשמודל אינו זמין (חריגה ממכסה, rate-limit, מודל שהוצא משימוש, או תקלת ספק), הבוט
עובר **אוטומטית** למודל החינמי הבא ברשימה, **שולח לך הודעה על המעבר**, ושומר את
המודל החדש כפעיל. שרשראות ה-fallback:

- טקסט: `deepseek → llama → qwen → gemini`
- ראייה: `gemini → llama-3.2-vision → qwen-vl`

מודל שכבר לא קיים פשוט מחזיר שגיאה והלולאה ממשיכה הלאה — כך שהשרשרת מרפאת את עצמה.
אפשר לחזור ידנית בכל רגע עם `/model <שם>`.

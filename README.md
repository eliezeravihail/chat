...
אחרת הבוט ינסה לענות גם על התראות הניטור. שם ה-topic הוא הסוד היחיד; בחר משהו
ארוך ואקראי (או הרם שרת ntfy עצמאי). ל-ntfy.sh יש מגבלות קצב לשימוש חינמי.
...
| `MEMORY_KEY` | — | אם מוגדר — הזיכרון נשמר **מוצפן** ב-Redis (Fernet/AES); בלעדיו — טקסט גלוי |
...
**הצפנת הזיכרון (מומלץ):** תוכן השיחה רגיש. הגדר secret נוסף בשם **`MEMORY_KEY`**
(מחרוזת אקראית חזקה, למשל `openssl rand -base64 32`) — ואז היסטוריית השיחה
נשמרת ב-Redis **מוצפנת** (Fernet = AES מאומת), ומפוענחת רק בתוך התהליך של הבוט
כשבונים את הקונטקסט למודל. כך גם לספק ה-Redis (Upstash) יש רק ciphertext.
⚠️ שמור על המפתח — אם תאבד אותו, ההיסטוריה הקיימת לא ניתנת לפענוח (הבוט פשוט
יתחיל היסטוריה חדשה, בלי לקרוס).

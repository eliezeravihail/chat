...
**נפילה אוטומטית למודלים חינמיים**, אישיות **פסיכותרפיסט בשם משה** בעברית
...
## מעבר מ-Twilio ל-Hermes

כשמעבירים את `DEPLOY_TARGET` ל-`hermes` ומריצים פריסה, שלב ה-Hermes **מכבה
אוטומטית את שירות `wa-bot`** (Twilio) ומרים את Hermes במקומו. אין צורך לכבות
ידנית. (רוצה לחזור? החזר את המשתנה ל-`twilio`.)

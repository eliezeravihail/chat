...
## ערוץ ntfy פרטי (לבעלים, מהנייד) + הצפנה

הרצת `hermes/setup-ntfy-server.sh` מקימה **שרת ntfy משלנו על אותו VM**, עם
HTTPS ציבורי (דומיין חינמי מ-DuckDNS + תעודה אוטומטית של Caddy) — ואת הכתובת
מזינים באפליקציית ntfy הקיימת בטלפון. אין צורך להתקין שום דבר בטלפון.

- **בלי נושאים אקראיים:** ההגנה היא **שם משתמש + סיסמה** (השרת דוחה כל גישה
  אחרת — `deny-all`), ולכן שמות הנושאים פשוטים וקבועים: `chat` (אתה כותב)
  ו-`bot` (הבוט עונה). מזינים את הסיסמה פעם אחת באפליקציה.
- דרישות חד-פעמיות: subdomain חינמי ב-<https://duckdns.org> (2 דק'), וסימון
  Allow HTTP/HTTPS על ה-VM ב-GCP.

**מפת הצפנה של כל המערכת:**

| קטע | הצפנה |
| --- | --- |
| וואטסאפ: טלפון החבר ⇄ ה-VM | קצה-לקצה (Signal protocol) — גם Meta לא רואה |
| ntfy: הטלפון שלך ⇄ השרת **שלנו** | HTTPS ‏(Let's Encrypt) + סיסמה; אין צד שלישי בדרך |
| Hermes ⇄ ntfy על ה-VM | localhost — לא יוצא לרשת כלל |

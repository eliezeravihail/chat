...
## ערוץ ntfy (לבעלים, מהנייד) + הצפנה

הרצת `hermes/setup-ntfy-server.sh` מוסיפה ערוץ **ntfy** לשיחה עם הסוכן מהנייד —
מתאים גם לטלפון מסונן (בלי VPN): משתמשים ב-**ntfy.sh הציבורי** עם שמות נושאים
**אקראיים ארוכים** שמתפקדים כסיסמה בלתי-ניתנת-לניחוש. באפליקציית ntfy נרשמים
לנושא ה-out (תשובות) ומפרסמים לנושא ה-in (שאלות). החלפת נושאים: `NTFY_REGEN=1`.
...
| ntfy: הטלפון שלך ⇄ ntfy.sh ⇄ ה-VM | TLS בדרך; שרת ntfy.sh רואה תוכן (פשרת נוחות) |
| ה-VM ⇄ OpenRouter/מודל | TLS; ספק המודל מעבד את הטקסט (בלתי נמנע בכל בוט LLM) |

לפרטיות מלאה ב-ntfy (אופציה B): ntfy עצמי על ה-VM מאחורי HTTPS עם דומיין חינמי
(DuckDNS + Caddy) — דורש שספק הסינון של הטלפון יאשר את הדומיין. לא ממוטב
אוטומטית; בקש ואבנה כשתרצה.
